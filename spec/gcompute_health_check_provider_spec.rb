...
describe Puppet::Type.type(:gcompute_health_check).provider(:google) do
        ...
        context 'title == name (pass)' do
          ...
          subject do
            apply_compiled_manifest(
              ...
              MANIFEST
            ).catalog.resource('Gcompute_health_check[title0]').provider.ensure
          end
        ...
        context 'title == name (pass)' do
          ...
          subject do
            apply_compiled_manifest(
              ...
              MANIFEST
            ).catalog.resource('Gcompute_health_check[title0]')
              .provider.ensure
          end
  ...
  context '#flush' do
    subject do
      Puppet::Type.type(:gcompute_health_check).new(
        ensure: :present,
        name: 'my-name'
      ).provider
    end
  ...
  def create_type(id)
    Puppet::Type.type(:gcompute_health_check).new(
      ...
      project: HC_PROJECT_DATA[(id - 1) % HC_PROJECT_DATA.size],
      name: HC_NAME_DATA[(id - 1) % HC_NAME_DATA.size]
    )
  end

  def expand_variables(template, data, extra_data = {})
    Puppet::Type.type(:gcompute_health_check).provider(:google)
