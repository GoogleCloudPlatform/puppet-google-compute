...
describe Puppet::Type.type(:gcompute_global_address).provider(:google) do
  ...
  context '#flush' do
    subject do
      Puppet::Type.type(:gcompute_global_address).new(
        ensure: :present,
        name: 'my-name'
      ).provider
    end
  ...
  def create_type(id)
    Puppet::Type.type(:gcompute_global_address).new(
      ...
      project: GA_PROJECT_DATA[(id - 1) % GA_PROJECT_DATA.size],
      name: GA_NAME_DATA[(id - 1) % GA_NAME_DATA.size]
    )
  end

  def expand_variables(template, data, extra_data = {})
    Puppet::Type.type(:gcompute_global_address).provider(:google)
