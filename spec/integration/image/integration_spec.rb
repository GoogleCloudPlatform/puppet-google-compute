...
describe 'image.create', vcr: true do
  it 'creates and destroys non-existent image' do
    ...
    VCR.use_cassette('check_image') do
      validate_no_flush_calls('image')
    end
    puts 'destroying image'
    VCR.use_cassette('destroy_image') do
      run_example('delete_image')
    end
    puts 'confirming image destroyed'
    VCR.use_cassette('check_destroy_image') do
      validate_no_flush_calls('delete_image')
    end
  end
end
